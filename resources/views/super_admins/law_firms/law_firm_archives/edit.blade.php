...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-4 pt-4 pt-lg-0">
            ...
            <div class="col-sm-6">
                <h2 class="main-content-title fw-bold mb-0">Archive</h2>
                <ol class="breadcrumb float-sm-left">
                    <li class="breadcrumb-item"><a href="{{ route('super_admin.dashboard') }}">Home</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('super_admin.law_firms.index') }}">LawFirm </a></li>
                    <li class="breadcrumb-item"><a href="{{ route('super_admin.law_firm_archives.index' , $law_firm->id) }}">Archives </a></li>
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <div class="card card-secondary">
                    ...
                    <!-- form start -->
                    <form id="quickForm" method="POST" action="{{ route('super_admin.law_firm_archives.update', ['law_firm' => $law_firm->id , 'law_firm_archive' => $law_firm_archive->id]) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                        ...
                        <div class="row">
                                    ...
                                    <div class="tab-content mt-2 p-2" id="myTabContent">
                                        ...
                                        <div class="tab-pane fade mb-2 {{$key == 0 ? 'show active':''}}" id="nav-{{$language->code}}" role="tabpanel" aria-labelledby="nav-{{$language->code}}-tab">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="name[{{$language->code}}]" value="{{ $law_firm_archive->getTranslation('name', $language->code); }}" class="form-control @if ($errors->has('name.'.$language->code)) is-invalid @endif" id="InputName-{{$language->code}}" placeholder="Please Enter" aria-describedby="NameError-{{$language->code}}" aria-invalid="true">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea name="description[{{$language->code}}]" id="discription_editor_{{$language->code}}" class="form-control @if ($errors->has('description.'.$language->code)) is-invalid @endif" rows="3" placeholder="Please Enter" aria-describedby="DescriptionError-{{$language->code}}" aria-invalid="true">{{ $law_firm_archive->getTranslation('description', $language->code); }}</textarea>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 col-lg-4">
                                    <div class="form-group">
                                        <label for="InputName">Select Archive Category :</label>
                                        <select aria-describedby="PostCategoryError" aria-invalid="true" id="faq_categories_drop_down" class="form-control curr-sym @if ($errors->has('archive_category_id')) is-invalid @endif" name="archive_category_id">
                                            @if (count($archive_categories) > 0)
                                            <option value="">Select Archive Category</option>
                                            @foreach ($archive_categories as $archive_category)
                                            <option value="{{ $archive_category->id }}" {{ $law_firm_archive->archive_category_id == $archive_category->id ? 'selected' : '' }}>
                                                {{ $archive_category->name }}
                                            </option>
                                            @endforeach
                                            @else
                                            <option value="">No Archive Category Exists</option>
                                            @endif
                                        </select>
                                ...
                                <div class="col-md-12 col-lg-4">
                                    <div class="form-group">
                                        <label for="InputName">Select Archive Tags :</label>
                                        <select multiple aria-describedby="TagsError" aria-invalid="true" id="tags_drop_down" class="form-control curr-sym @if ($errors->has('tags')) is-invalid @endif" name="tags[]">
                                            @if (count($tags) > 0)
                                            <option value="">Select Archive Tag</option>
                                            @foreach ($tags as $tag)
                                            <option value="{{ $tag->id }}">
                                                {{ $tag->name }}
                                            </option>
                                            @endforeach
                                            @else
                                            <option value="">No Archive Tag Exists</option>
                                            @endif
                                        </select>
                                ...
                                <div class="col-md-12 col-lg-4">
                                    <div class="form-group">
                                        ...
                                        @if ($law_firm_archive->image)
                                        <div class="custom-file-preview">
                                            <img src="{{ url($law_firm_archive->image) }}" width="75px" height="75px" alt="{{ $law_firm_archive->name }}">
                                        </div>
                                        @else
                                        <div class="custom-file-preview">
                                            -- No Image Selected
                                ...
                                <div class="col-md-12 col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <div class="custom-control custom-switch">
                                            <input type="checkbox" name="is_active" value="1" @if ($law_firm_archive->is_active) checked @endif class="custom-control-input"
                                            id="customSwitch1" aria-describedby="IsActiveError" aria-invalid="true">
                                            <label class="custom-control-label" for="customSwitch1">Select Archive To Be
                                                Active
                                                Or Not</label>
                                        </div>
