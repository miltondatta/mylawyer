...
class AppointmentScheduleController extends Controller
{
    ...
    public function saveAppointmentSchedule(CreateRequest $request)
    {
        ...
        if ($request->is_schedule_required) {
            $records = AppointmentSchedule::where('law_firm_id', $law_firm_id)->where('appointment_type_id', $request->appointment_type_id)->get();
            ...
            AppointmentSchedule::where('law_firm_id', $law_firm_id)->where('appointment_type_id', $request->appointment_type_id)->delete();
            if ($request->fee) {
